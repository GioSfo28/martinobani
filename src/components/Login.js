...
// Cambiate le icone per uno stile più pulito, mantenendo l'essenziale
...
const LoginPage = () => {
  ...
  // Auto-focus e scroll to top quando la pagina si carica
  useEffect(() => {
  ...
  // Logica per caricare i permessi dal database (INVARIATA)
  useEffect(() => {
    ...
    get(permissionsRef)
      .then((snapshot) => {
        if (snapshot.exists()) {
          ...
          // Manteniamo la logica di estrazione email
  ...
  // Logica di accesso con Google (INVARIATA)
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!authorizedEmails.includes(user.email)) {
        setError("Utente non autorizzato: chiedi l'autorizzazione a giosfo28@gmail.com");
      ...
      if (!snapshot.exists()) {
        console.log("Utente non trovato nel DB. Creazione nuovi dati utente.");
        const userData = {
          email: user.email,
          uid: user.uid,
        ...
        console.log("Dati utente creati:", userData);
      } else {
  ...
  // ------------------------------------------------------------------------
  // NUOVO MARKUP / STILE (Più sobrio e professionale)
  // ------------------------------------------------------------------------
  return (
    <>
      <Header />
